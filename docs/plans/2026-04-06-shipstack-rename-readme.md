# shipstack Rename + README Rewrite Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rename the project to `shipstack` and rewrite README.md to a concise, marketing-friendly format targeting indie hackers.

**Architecture:** Two independent changes — (1) rename `package.json` name field, (2) replace README content. No code changes, no file moves.

**Tech Stack:** Markdown, JSON.

---

### Task 1: Rename project to shipstack
...
In `/Users/bytedance/Projects/orpc_template/package.json`, change:
```json
"name": "orpc-template",
```
to:
...
```
Expected: `"name": "shipstack",`

**Step 3: Commit**

```bash
git add package.json
git commit -m "chore: rename project to shipstack"
```
...
Replace the entire content of `/Users/bytedance/Projects/orpc_template/README.md` with:
...
wc -l README.md
```
Expected: roughly 80–100 lines (vs current 272).

**Step 3: Commit**

```bash
git add README.md
git commit -m "docs: rewrite README for shipstack — concise indie hacker style"
```
